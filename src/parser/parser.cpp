...
StreamParser::StreamParser(Options opts) : opts_(opts) {
    buf_.reserve(65536);
}
...
std::size_t StreamParser::parse_field(const char* data, std::size_t len,
                                       TagNum& tag, std::string_view& value) {
    // Find '=' separator
    std::size_t eq = 0;
    while (eq < len && data[eq] != '=' && data[eq] != SOH) ++eq;
    if (eq >= len || data[eq] != '=') return 0; // need more data

    // Parse tag number
    TagNum t{};
    auto [ptr, ec] = std::from_chars(data, data + eq, t);
    if (ec != std::errc{} || ptr != data + eq) return 0; // bad tag

    // Find value end (SOH)
    std::size_t soh = eq + 1;
    while (soh < len && data[soh] != SOH) ++soh;
    if (soh >= len) return 0; // need more data
...
bool StreamParser::try_parse_one() {
    ...
    if (avail < 20) return false; // minimum viable message

    // We expect the message to start with tag 8 (BeginString)
    // Quick check
    if (avail < 5 || base[0] != '8' || base[1] != '=') return false;
    ...
    if (consumed == 0 || tag != tags::BeginString) return false;
    std::string begin_string(value);
    pos += consumed;

    // -- Tag 9 (BodyLength) --------------------------------------------------
    consumed = parse_field(base + pos, avail - pos, tag, value);
    if (consumed == 0 || tag != tags::BodyLength) return false;
    ...
    if (avail < needed) return false; // wait for more data
    ...
    while (pos < avail) {
        consumed = parse_field(base + pos, avail - pos, tag, value);
        if (consumed == 0) return false; // need more data
