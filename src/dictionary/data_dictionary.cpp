...
FieldType parse_field_type(std::string_view sv) noexcept {
    if (sv == "INT"    || sv == "SEQNUM"  || sv == "NUMINGROUP" ||
        sv == "LENGTH" || sv == "TAGNUM") return FieldType::Int;
    if (sv == "FLOAT"  || sv == "PRICE"   || sv == "AMT"        ||
        sv == "PRICEOFFSET" || sv == "QTY"|| sv == "PERCENTAGE") return FieldType::Float;
    if (sv == "CHAR")                     return FieldType::Char;
    if (sv == "STRING" || sv == "EXCHANGE"|| sv == "COUNTRY"    ||
        sv == "CURRENCY")                 return FieldType::String;
    if (sv == "DATA")                     return FieldType::Data;
    if (sv == "BOOLEAN")                  return FieldType::Boolean;
    if (sv == "UTCTIMESTAMP")             return FieldType::UTCTimestamp;
    if (sv == "UTCTIMEONLY")              return FieldType::UTCTimeOnly;
    if (sv == "UTCDATEONLY")              return FieldType::UTCDateOnly;
    if (sv == "LOCALMKTDATE")             return FieldType::LocalMktDate;
    if (sv == "MONTHYEAR")                return FieldType::MonthYear;
    if (sv == "DAYOFMONTH")               return FieldType::DayOfMonth;
    if (sv == "MULTIPLEVALUESTRING")      return FieldType::MultipleValueString;
...
std::string_view to_string(FieldType t) noexcept {
    switch (t) {
        ...
        case FieldType::Data:        return "DATA";
        case FieldType::Boolean:     return "BOOLEAN";
        case FieldType::UTCTimestamp:return "UTCTIMESTAMP";
        case FieldType::NumInGroup:  return "NUMINGROUP";
        case FieldType::Qty:         return "QTY";
        case FieldType::Price:       return "PRICE";
...
void DataDictionary::load_builtin_fields() {
    ...
    auto add = [&](TagNum tag, std::string_view name, FieldType type,
        ...
        d.name = name;
        d.type = type;
        for (auto& v : values) d.valid_values.insert(std::string(v));
...
void DataDictionary::load_builtin_messages_42() {
    ...
    add(Reject, "Reject", true, {RefSeqNum}, {RefTagID, RefMsgType, tags::SessionRejectReason, Text});
    add(SequenceReset, "SequenceReset", true, {NewSeqNo}, {GapFillFlag});
    add(Logout, "Logout", true, {}, {Text});
    add(Logon, "Logon", true,
    ...
    add(NewOrderSingle, "NewOrderSingle", false,
        ...
        {Account, SecurityID, SecurityIDSource, OrdType, fix::tags::Price, StopPx,
         Currency, TimeInForce, ExecInst, Text});
    ...
    add(OrderCancelRequest, "OrderCancelRequest", false,
        {OrigClOrdID, ClOrdID, Symbol, Side, TransactTime},
        {Account, Text});
    ...
    add(MarketDataIncrementalRefresh, "MarketDataIncrementalRefresh", false,
        {NoMDEntries}, {MDReqID});
    add(BusinessMessageReject, "BusinessMessageReject", false,
        {RefMsgType, BusinessRejectReason}, {RefSeqNum, Text});
...
void DataDictionary::load_builtin(FixVersion version) {
    ...
    load_builtin_fields();
    switch (version) {
        case FixVersion::FIX_4_2:    load_builtin_messages_42(); break;
        case FixVersion::FIX_4_4:    load_builtin_messages_44(); break;
        case FixVersion::FIX_5_0SP2:
        case FixVersion::FIXT_1_1:   load_builtin_messages_50sp2(); break;
        default:                     load_builtin_messages_44(); break;
    }
    {
        std::unique_lock lock(mutex_);
        version_string_ = std::string(fix::to_string(version));
...
Result<void> DataDictionary::validate(const Message& msg) const {
    ...
    for (const auto& f : msg.fields()) {
        ...
        if (it == fields_by_tag_.end()) continue;
...
FixVersion DictionaryRegistry::resolve_appl_ver_id(std::string_view id) noexcept {
    if (id == appl_ver_id::FIX42)    return FixVersion::FIX_4_2;
    if (id == appl_ver_id::FIX43)    return FixVersion::FIX_4_3;
    if (id == appl_ver_id::FIX44)    return FixVersion::FIX_4_4;
    if (id == appl_ver_id::FIX50)    return FixVersion::FIX_5_0;
    if (id == appl_ver_id::FIX50SP1) return FixVersion::FIX_5_0SP1;
    if (id == appl_ver_id::FIX50SP2) return FixVersion::FIX_5_0SP2;
