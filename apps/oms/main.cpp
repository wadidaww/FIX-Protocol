...
public:
    ...
    void run(const std::string& host, std::uint16_t port) {
        ...
        cbs.on_logout = [](const SessionID& sid, std::string_view reason) {
            std::cout << "[OMS] Logged out: " << sid.to_string()
                      << " reason=" << reason << "\n";
        };
        ...
        cbs.on_sequence_gap = [](const SessionID& sid, SeqNum exp, SeqNum got) {
            std::cout << "[OMS] SeqGap on " << sid.to_string()
        ...
        session_ = engine_->add_session(std::move(cfg), std::move(transport),
                                         std::move(cbs),
...
private:
    ...
    void send_test_order() {
        if (!session_ || !session_->is_active()) return;
    ...
    void handle_message(const SessionID& sid, const Message& msg) {
        ...
        if (mt == msg_types::ExecutionReport) {
            ...
            std::cout << "[OMS] ExecReport ordID=" << ordid
                      << " clOrdID=" << clordid
                      << " status=" << status
                      << " execType=" << exec_type
                      << " leaves=" << leaves
            ...
            std::cout << "[OMS] CancelReject clOrdID=" << clordid
                      << " reason=" << reason << "\n";
